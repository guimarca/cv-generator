...
from cv_generator.utils import FILES_PATH

env = Environment(
    loader=FileSystemLoader(FILES_PATH), autoescape=select_autoescape(["html"])
)
...
def generate_html(cv: dict, full=False):
    ...
    output_path = f"{os.path.join(FILES_PATH, get_output_name(cv))}.html"
